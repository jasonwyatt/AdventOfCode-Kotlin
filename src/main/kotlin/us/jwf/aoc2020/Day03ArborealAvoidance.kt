...
/**
 * Day three of AoC 2020.
 */
class Day03ArborealAvoidance : Day<Long, Long> {
  /**
  ...
  override suspend fun executePart1(input: Reader): Long {
    val chars = input.toLineFlow().map { it.toCharArray() }.toList()
    return chars.countTrees(3, 1)
  }
  ...
  override suspend fun executePart2(input: Reader): Long {
    val chars = input.toLineFlow().map { it.toCharArray() }.toList()
    return chars.countTrees(1, 1) *
      chars.countTrees(3, 1) *
      chars.countTrees(5, 1) *
      chars.countTrees(7, 1) *
      chars.countTrees(1, 2)
  }

  private fun List<CharArray>.countTrees(right: Int, down: Int): Long {
    var positionX = 0
    var positionY = 0
    var count = 0L
    while (positionY < size) {
      if (this[positionY][positionX] == '#') count++
      positionX += right
      positionX %= this[0].size
      positionY += down
    }
    return count
  }
