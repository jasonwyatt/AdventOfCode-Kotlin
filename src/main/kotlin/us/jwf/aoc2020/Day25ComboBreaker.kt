...
class Day25ComboBreaker : Day<Long, Int> {
   ...
   * breath, you describe the status of the elevator and the exact number of stairs you just had
   * to climb.
   *
   ...
   *
   * * The card transforms the subject number of 7 according to the card's secret loop size. The
   *   result is called the card's public key.
   * * The door transforms the subject number of 7 according to the door's secret loop size. The
   *   result is called the door's public key.
   * * The card and door use the wireless RFID signal to transmit the two public keys (your puzzle
   *   input) to the other device. Now, the card has the door's public key, and the door has the
   *   card's public key. Because you can eavesdrop on the signal, you have both public keys, but
   ...
   * error, you can work out that the card's loop size must be 8, because transforming the initial
   ...
   * determine that the door's loop size is 11, because transforming the initial subject number of
   * 7 with a loop size of 11 produces 17807724.
   *
   * At this point, you can use either device's loop size with the other device's public key to
   * calculate the encryption key. Transforming the subject number of 17807724 (the door's public
   * key) with a loop size of 8 (the card's loop size) produces the encryption key, 14897079.
  ...
  override suspend fun executePart1(input: Reader): Long {
    val (cardPublicKey, doorPublicKey) = input.readLines().map { it.toLong() }
    ...
    var keyValue = 1L
    repeat(cardLoopSize) {
      keyValue *= doorPublicKey
      keyValue %= 20201227
    }
    return keyValue
  }

  override suspend fun executePart2(input: Reader): Int = 1337
}