 ...
 * Day 21 of AoC 2020
 */
class Day21AllergenAssessment : Day<Int, String> {
  /**
   ...
   * sometimes, allergens are listed in a language you do understand. You should be able to use
   * this information to determine which ingredient contains which allergen and work out which
   ...
   * shellfish) after an ingredients list), the ingredient that contains each listed allergen will
   * be somewhere in the corresponding ingredients list. However, even if an allergen isn't listed,
   ...
   * in any food in your list. In the above example, none of the ingredients kfcds, nhms, sbzzf, or
   * trh can contain an allergen. Counting the number of times any of these ingredients appear in
   * any ingredients list produces 5: they all appear once each except sbzzf, which appears twice.
   *
  ...
  override suspend fun executePart1(input: Reader): Int {
    val (allergenSuspects, allIngredients) = input.getData()
    val allergenIngredients = allergenSuspects.values.flatMap { it.toList() }.toSet()
    var totalNonAllergens = 0
    allIngredients.forEach { ingredient, count ->
      if (ingredient !in allergenIngredients) totalNonAllergens += count
    }
    return totalNonAllergens
  }
  ...
  override suspend fun executePart2(input: Reader): String {
    var (allergenSuspects, _) = input.getData()

    while (allergenSuspects.values.any { it.size > 1 }) {
      val newSuspects = mutableMapOf<String, Set<String>>()

      allergenSuspects.forEach { (allergen, suspects) ->
        val suspectsSoFar = suspects.toMutableSet()
        allergenSuspects.forEach others@{ (otherAllergen, otherSuspects) ->
          if (allergen == otherAllergen) return@others
          if (otherSuspects.size == 1) {
            suspectsSoFar -= otherSuspects
          }
        }
        newSuspects[allergen] = suspectsSoFar
      }

      allergenSuspects = newSuspects
    }

    return allergenSuspects.entries
      .sortedBy { it.key }
      .flatMap { it.value }
      .joinToString(",")
  }

  private fun <K, V> Map<K, V>.print() {
    println("{")
    forEach { k, v -> println("  $k: $v,") }
    println("}")
  }

  private suspend fun Reader.getData(): Pair<Map<String, Set<String>>, Map<String, Int>> {
    val allergenSuspects = mutableMapOf<String, Set<String>>()
    val allIngredients = mutableMapOf<String, Int>()

    toLineFlow().collect {
      val match = pattern.matchEntire(it) ?: return@collect
      val ingredients = match.groupValues[1].split(" ")
        .onEach { ingredient ->
          allIngredients[ingredient] = (allIngredients[ingredient] ?: 0) + 1
        }
        .toSet()
      match.groupValues[2].split(", ").forEach { allergen ->
        allergenSuspects[allergen] =
          (allergenSuspects[allergen] ?: ingredients) intersect ingredients
      }
    }

    return allergenSuspects to allIngredients
  }

  companion object {
    private val pattern = Regex("([a-z ]+) \\(contains ([a-z]+(, [a-z]+)*)\\)")
  }
}