 ...
 * Day 15 of AoC 2020.
 */
class Day15RambunctiousRecitation : Day<Int, Int> {
  /**
  ...
  override suspend fun executePart1(input: Reader): Int {
    val numbers = input.toIntFlow(",").toList()
    return iterate(numbers, 2020)
  }
  ...
  override suspend fun executePart2(input: Reader): Int {
    val numbers = input.toIntFlow(",").toList()
    return iterate(numbers, 30000000)
  }

  private fun iterate(startingNumbers: List<Int>, iterations: Int): Int {
    val lastHeard = mutableMapOf<Int, Int>()
    startingNumbers.forEachIndexed { index, i -> lastHeard[i] = index + 1 }
    lastHeard.remove(startingNumbers.last())

    var numbersSoFar = startingNumbers.size
    var lastNumberRead = startingNumbers.last()
    while (numbersSoFar < iterations) {
      val lastTimeRead = lastHeard[lastNumberRead] ?: 0
      val currentNumberRead = if (lastTimeRead != 0) numbersSoFar - lastTimeRead else 0
      lastHeard[lastNumberRead] = numbersSoFar
      lastNumberRead = currentNumberRead
      numbersSoFar++
    }
