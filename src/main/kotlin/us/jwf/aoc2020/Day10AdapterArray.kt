 ...
 * Day 10 of AoC 2020
 */
class Day10AdapterArray : Day<Int, Long> {
  /**
   ...
   *   to it directly would need to have a joltage rating of 1, 2, or 3 jolts. Of these, only one
  ...
  override suspend fun executePart1(input: Reader): Int {
    val values = input.toIntFlow().toList().sorted()
    var oneDiffs = 0
    var threeDiffs = 0
    values.withIndex().fold(0) { acc, (index, value) ->
      when (value - acc) {
        1 -> oneDiffs++
        3 -> threeDiffs++
      }
    ...
    // Add another three-diff for the actual device.
   ...
   *
   * The second example above (the one that starts with 28, 33, 18) has many arrangements. Here are
   * a few:
   *
   * ```
  ...
  override suspend fun executePart2(input: Reader): Long {
    val values = input.toIntFlow().toList().sorted()
    val deviceJolts = values.last() + 3
    val counts = LongArray(deviceJolts + 1) { if (it == 0) 1 else 0 }

    for (i in values) {
      when (i) {
          1 -> counts[i] = 1
          2 -> counts[i] = counts[i - 1] + counts[i - 2]
          else -> counts[i] = counts[i - 1] + counts[i - 2] + counts[i - 3]
      }
    }

    return  counts[deviceJolts - 1] + counts[deviceJolts - 2] + counts[deviceJolts - 3]
  }
}