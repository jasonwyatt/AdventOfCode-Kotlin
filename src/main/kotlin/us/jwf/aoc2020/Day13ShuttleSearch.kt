 ...
 * Day 13 of AoC
 */
class Day13ShuttleSearch : Day<Int, Long> {
  /**
   ...
   * sea port. After that, each bus travels to the airport, then various other locations, and
   ...
   * the sea port at timestamps 0, 5, 10, 15, and so on. The bus with ID 11 departs at 0, 11, 22,
  ...
  override suspend fun executePart1(input: Reader): Int {
    val lines = input.toLineFlow().toList()
    val earliestTime = lines[0].toInt(10)
    val (bus, wait) = lines[1].split(",").asSequence()
      .mapNotNull { it.toIntOrNull(10) }
      .map { it to it - (earliestTime % it) }
  ...
  override suspend fun executePart2(input: Reader): Long {
    val lines = input.toLineFlow().toList()
    val offsetsAndBusses = lines[1].split(",").asSequence()
      .withIndex()
      .mapNotNull { (index, str) -> str.toLongOrNull()?.let { index to it } }
      .toList()

    var t: Long = 1
    while (true) {
      val matches = offsetsAndBusses.filter { (offset, bus) -> (t + offset) % bus == 0L }
      if (matches.size == offsetsAndBusses.size) return t

      val stepSize = matches.fold(1L) { acc, (_, bus) -> acc * bus }
      t += stepSize
    }
  }
}