...
class DayTwo : Day<Int, Int> {
  ...
  override suspend fun executePart1(input: Reader): Int {
    return input.toMatchFlow(pattern)
      .map { it.toPart1PolicyAndPassword() }
      .filter { (policy, pw) ->
        val policyCharCount =
          pw.toCharArray().asSequence()
            .filter { it == policy.targetChar }
            .count()
        val valid = policyCharCount in policy.minAppearances..policy.maxAppearances
        valid
      }
      .count()
  }
   ...
   * letter. Other occurrences of the letter are irrelevant for the purposes of policy
   * enforcement.
   *
  ...
  override suspend fun executePart2(input: Reader): Int {
    return input.toMatchFlow(pattern)
      .map { it.toPart2PolicyAndPassword() }
      .filter { (policy, pw) ->
        val firstButNotSecond =
          pw[policy.firstPosition] == policy.targetChar &&
            pw[policy.secondPosition] != policy.targetChar
        val secondButNotFirst =
          pw[policy.secondPosition] == policy.targetChar &&
            pw[policy.firstPosition] != policy.targetChar

        firstButNotSecond || secondButNotFirst
      }
      .count()
  }

  private fun List<String>.toPart1PolicyAndPassword(): Pair<Part1Policy, String> {
    val policy = Part1Policy(
      minAppearances = this[2].toInt(10),
      maxAppearances = this[3].toInt(10),
      targetChar = this[4].first()
    )
    return policy to this[5]
  }

  private fun List<String>.toPart2PolicyAndPassword(): Pair<Part2Policy, String> {
    val policy = Part2Policy(
      firstPosition = this[2].toInt(10) - 1,
      secondPosition = this[3].toInt(10) - 1,
      targetChar = this[4].first()
    )
    return policy to this[5]
  }

  private data class Part1Policy(
    val minAppearances: Int,
    val maxAppearances: Int,
    val targetChar: Char
  )

  private data class Part2Policy(
