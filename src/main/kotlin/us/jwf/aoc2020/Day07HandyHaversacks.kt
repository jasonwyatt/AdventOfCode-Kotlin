 ...
 * Day 7 of AoC 2020
 */
class Day07HandyHaversacks : Day<Int, Int> {
  /**
   ...
   * bag is empty, every vibrant plum bag contains 11 bags (5 faded blue and 6 dotted black), and
   * so on.
   *
  ...
  override suspend fun executePart1(input: Reader): Int {
    ...
    graph.traverseFrom("shiny gold") { name, _ ->
      set.add(name)
      true
    }
  ...
  override suspend fun executePart2(input: Reader): Int {
    ...
    fun sumTree(root: Node<String>): Double {
      val outgoing = graph.edgesForNode(root)
      return 1 + outgoing.map { it.weight * sumTree(it.to) }.sum()
    }

    return sumTree(Node("shiny gold")).toInt() - 1
  }

  companion object {
    private val PATTERN =
      Regex("(([a-z]+ [a-z]+) bags) contain ((((\\d+) ([a-z]+ [a-z]+) bag(s)?)(, )?)+)\\.")
  }
}