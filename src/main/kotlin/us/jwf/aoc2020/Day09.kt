...
class Day09 : Day<Long, Long> {
  ...
  override suspend fun executePart1(input: Reader): Long {
    val values = input.toLongFlow().toList()
    (25 until values.size).forEach { index ->
      var found = false
      ...
      for (i in 1..25) {
        ...
        for (j in (i + 1)..25) {
          if (values[index - i] + values[index - j] == values[index]) {
            found = true
            break
          }
        }
  ...
  override suspend fun executePart2(input: Reader): Long {
    val values = input.toLongFlow().toList()
    val prefixSums = values.scan(0L) { acc, value -> acc + value }.drop(1).toLongArray()
    val (start, end) = requireNotNull(prefixSums.findSequenceBounds(70639851L))
    val (min, max) = values.findMinAndMax(from = start, to = end)
    return min + max
  }

  /**
   * Given an array of Longs generated as prefix sums, attempts to find the indices of the array
   * where the subsequence within the original array between those indices sums to the [target].
   *
   * NOTE: This could be done faster with a lookup, instead of using binary search.
   */
  private fun LongArray.findSequenceBounds(target: Long): Pair<Int, Int>? {
    for (i in 0 until (size - 1)) {
      val index = binarySearch(target + this[i], i + 1)
      if (index > 0) return i + 1 to index
    }
    return null
  }

  private fun List<Long>.findMinAndMax(from: Int, to: Int): Pair<Long, Long> {
