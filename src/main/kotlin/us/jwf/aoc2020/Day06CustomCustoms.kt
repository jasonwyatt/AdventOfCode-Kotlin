 ...
 * Day 6 of AoC 2020
 */
class Day06CustomCustoms : Day<Int, Int> {
  /**
   ...
   * The second group contains three people; combined, they answered "yes" to 3 questions: a, b, and
   *   c.
  ...
  override suspend fun executePart1(input: Reader): Int {
    return input.toLineFlow("\n\n")
      .map { group ->
        val options = LETTERS.toMutableSet()
        group.forEach { options.remove(it) }
        26 - options.size
      }
      .fold(0) { a, b -> a + b }
  }
      ...
      .map { group ->
        ...
        group.forEach {
          if (it == '\n') peopleInGroup++
            ...
            letters[it] = letters.getOrDefault(it, 0) + 1
          }
        }
        letters.values.filter { it == peopleInGroup }.count()
      }
      .fold(0) { a, b -> a + b }
  }

  companion object {
    private val LETTERS = "abcdefghijklmnopqrstuvwxyz".toCharArray().toSet()
  }
}