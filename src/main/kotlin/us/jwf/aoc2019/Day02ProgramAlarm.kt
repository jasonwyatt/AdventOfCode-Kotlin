 ...
 * AoC 2019 Day 2
 */
class Day02ProgramAlarm : Day<Int, Int> {
  /**
   ...
   * finished and should immediately halt. Encountering an unknown opcode means something went
   * wrong.
   *
   ...
   * get 70. Then, store this value at the output position; here, the output position (3) is at
   ...
   * the value placed in address 2 is called the verb. Each of the two input values will be between
   * 0 and 99, inclusive.
   *
  ...
  override suspend fun executePart2(input: Reader): Int {
    ...
    while (queue.isNotEmpty()) {
      ...
      val arg2Up = arg1 to (arg2 + 1)
      if (arg1Up !in visited) {
        queue.addFirst(arg1Up)
        visited.add(arg1Up)
      }
      if (arg2Up !in visited) {
        queue.addFirst(arg2Up)
        visited.add(arg2Up)
      }

      computer.reset()
    }
    throw IllegalArgumentException("Goal state is unreachable")
  }
}