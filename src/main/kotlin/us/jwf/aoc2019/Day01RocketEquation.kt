 ...
 * AoC 2019 Day 1
 */
class Day01RocketEquation : Day<Int, Int> {
  /**
  ...
  override suspend fun executePart1(input: Reader): Int {
    return input.toIntFlow().map { it / 3 - 2 }.reduce { acc, value -> acc + value }
  }
  ...
  override suspend fun executePart2(input: Reader): Int {
    return input.toIntFlow()
      .map { componentWeight ->
        flow {
          var fuelWeight = componentWeight / 3 - 2
          while (fuelWeight > 0) {
            emit(fuelWeight)
            val newFuel = fuelWeight / 3 - 2
            fuelWeight = newFuel
          }
